...
try {
  ...
  const aliceSeesBobStream = waitFor(
    alice,
    "call:stream",
    (call, stream, peerId) => call.id === started.id && peerId === "bob",
  );
  const bobSeesAliceStream = waitFor(
    bob,
    "call:stream",
    (call, stream, peerId) => call.id === started.id && peerId === "alice",
  );
  ...
  const bobSeesScreenProducer = waitFor(
    bob,
    "sfu:producer-added",
    (call, event) => event.callId === started.id && event.peerId === "alice" && event.appData?.source === "screen",
  );
  const bobSeesScreen = waitFor(
    bob,
    "call:stream",
    (call, stream, peerId) => call.id === started.id && peerId === "alice" && stream.getVideoTracks().length > 0,
  );
  ...
  const bobSeesRemoved = waitFor(
    ...
    (call, event) => event.callId === started.id && event.peerId === "alice",
  );
  ...
  const daveSeesProducer = waitFor(
    dave,
    "sfu:producer-added",
    (call, event) => event.callId === rawStarted.id && event.peerId === "carol",
  );
  ...
  console.log(
    "✅ v0.7 client smoke test passed: auto-wired SFU calls publish and consume, and the explicit API still works.",
  );
  alice.disconnect();
  bob.disconnect();
  carol.disconnect();
